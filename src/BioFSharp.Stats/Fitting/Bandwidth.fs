...
module Bandwidth =
    ...
    let fromBinNumber min max nBins =
        let tmp = (float nBins)
    ...
    let forHistogram data =
        ...
        let dmin = fdata |> Seq.min
        let dmax = fdata |> Seq.max
    ...
    let scottNormal (data:float[]) =
        ...
        let ssd = StatisticalMeasure.stDev data
        (3.5 * ssd) / (xLength ** (1./3.))
    ...
    let freedmanDiaconis (data:float[]) =
        let xLength = float data.Length
        if (xLength < 2. ) then raise (System.ArgumentException("need at least 2 data points"))
        let iqr = (Percentiles.IQR MathNet.Numerics.Statistics.QuantileDefinition.Nist data) |> float
        (2. * iqr ) / (1./3.)
    ...
    let nrd0 (x:float[]) =
        ...
        if (xLength < 2 ) then raise (System.ArgumentException("need at least 2 data points"))
        let hi = StatisticalMeasure.stDev x        
        let iqrX = (Percentiles.IQR MathNet.Numerics.Statistics.QuantileDefinition.Nist x |> float) / 1.34 // # qnorm(.75) - qnorm(.25) = 1.34898
    ...
    let uLSCV (data:float[]) (numberOfBins:option<int>) =
        ...
        let hmax  = 1.144 * sqrt (StatisticalMeasure.varPopulation(data)) * n**(-1./5.)    
        let lower = 0.1 * hmax
        let upper = hmax
        let tol   = 0.1 * lower
        //band_den_bin

...
module Impute =
    ...
    let normal :  VectorBaseImputation<float> =          
        fun fdata index ->
            let median = BioFSharp.Stats.StatisticalMeasure.median fdata
            let std  = BioFSharp.Stats.StatisticalMeasure.stDevPopulation fdata
            if not(System.Double.IsNaN(median) || System.Double.IsNaN(std)) then
                let sample = new MathNet.Numerics.Distributions.Normal(median, std)
                sample.Sample()
            else
    ...
    let kNearestImpute k : MatrixBaseImputation<float[],float> = 
        fun data arr index ->
            ...
            let tmpArr =
                ...
                |> Array.map BioFSharp.Stats.StatisticalMeasure.mean
            tmpArr.[index]
